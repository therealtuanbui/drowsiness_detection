...
# Command to run: python predict_eye.py -img temp/sleeping-baby.jpg
...
if __name__ == "__main__":
    ...
    for rect in rects:
        ...
        showImage(leftEye, 150, 150)
        classLeft = predictImage(leftEye, model=model)
        if classLeft==0: print('Left: Closed')
        else: print('Left: Open')
        rightEyeRatio = shape[rStart:rEnd]
        rightEye = drawEyes(rightEyeRatio, image)
        showImage(rightEye, 150, 150)
        classRight = predictImage(rightEye, model=model)
        if classRight==0: print('Right: Closed')
        else: print('Right: Open')
