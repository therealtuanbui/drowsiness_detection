...
def generate_dataset(type, dirData):
    if type==0:
        dataset = np.ndarray([1193*2, 24, 24, 1], dtype='float32')
    else: dataset = np.ndarray([1232*2, 24, 24, 1], dtype='float32')
    i = 0
    for dir in dirData:
        for filename in os.listdir(dir):
            if filename.endswith('.jpg'):
                im = cv2.imread(dir + '/' + filename)
                # Convert to grayscale image using algorithm. Instead you can use cv2.cvtColor(im, cv2.COLOR_BGR2GRAY) 
...
if __name__ == '__main__':
    ...
    #FYI: https://stackoverflow.com/questions/8968884/python-serialization-why-pickle
